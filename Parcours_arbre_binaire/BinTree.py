...
class BinTree:
    ...
    def __init__(self,root=None,left=None,right=None):
        ...
        """
        # Conditions portant sur le type des arguments
...
def trigen(comp,T):
    """
    Trigen(comp,T) renvoie la liste T triée selon la relation d'ordre comp
    """
    for i in range (1,len(T)):
        #Insérer T[i] dans T[0..i-1]
        x=T[i]
        j=i-1
        while j>=0 and comp(T[j],x) :
            T[j+1]=T[j]
            j=j-1
        # j=-1 ou T[j] <= x
        T[j+1]=x
...
#test=BinTree(None,BinTree("A"),None)
"""
